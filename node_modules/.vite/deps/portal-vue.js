...
function bindObjectListeners(data3, value) {
  if (value) {
    if (!isPlainObject(value)) {
      ...
      var on = data3.on = data3.on ? extend({}, data3.on) : {};
      for (var key in value) {
        var existing = on[key];
        var ours = value[key];
...
function renderMixin(Vue2) {
  ...
  Vue2.prototype._render = function() {
    var vm = this;
    var _a = vm.$options, render4 = _a.render, _parentVnode = _a._parentVnode;
...
function createElement$1(context, tag, data3, children2, normalizationType, alwaysNormalize) {
  if (isArray(data3) || isPrimitive(data3)) {
...
function _createElement(context, tag, data3, children2, normalizationType) {
  if (isDef(data3) && isDef(data3.__ob__)) {
    warn("Avoid using observed data object as vnode data: ".concat(JSON.stringify(data3), "\n") + "Always create fresh vnode data objects in each render!", context);
  ...
  if (isDef(data3) && isDef(data3.key) && !isPrimitive(data3.key)) {
  ...
  if (typeof tag === "string") {
    ...
    if (config.isReservedTag(tag)) {
      if (isDef(data3) && isDef(data3.nativeOn) && data3.tag !== "component") {
...
function registerDeepBindings(data3) {
  if (isObject(data3.style)) {
    traverse(data3.style);
  }
  if (isObject(data3.class)) {
...
function createFunctionalComponent(Ctor, propsData, data3, contextVm, children2) {
  ...
  if (vnode instanceof VNode) {
    return cloneAndMarkFunctionalResult(vnode, data3, renderContext.parent, options, renderContext);
  } else if (isArray(vnode)) {
    var vnodes = normalizeChildren(vnode) || [];
    var res = new Array(vnodes.length);
    for (var i = 0; i < vnodes.length; i++) {
      res[i] = cloneAndMarkFunctionalResult(vnodes[i], data3, renderContext.parent, options, renderContext);
    }
    return res;
  }
}
function cloneAndMarkFunctionalResult(vnode, data3, contextVm, options, renderContext) {
  ...
  if (data3.slot) {
    (clone.data || (clone.data = {})).slot = data3.slot;
  }
  return clone;
}
...
function createComponent(Ctor, data3, context, children2, tag) {
  ...
  var propsData = extractPropsFromVNodeData(data3, Ctor, tag);
...
function assertObjectType(name2, value, vm) {
  if (!isPlainObject(value)) {
    warn('Invalid value for option "'.concat(name2, '": expected an Object, ') + "but got ".concat(toRawType(value), "."), vm);
  }
}
...
function initData(vm) {
  ...
  data3 = vm._data = isFunction(data3) ? getData(data3, vm) : data3 || {};
...
// ../node_modules/portal-vue/dist/portal-vue.esm.js
...
var portal_vue_esm_default = index2;
export {
  MountingPortal,
  Portal,
  PortalTarget,
  wormhole as Wormhole,
  portal_vue_esm_default as default
};
/*!
 * Vue.js v2.7.13
