    ...
    <div class="drawer-content has-background-black-ter has-text-white is-size-7 p-lg">
      <div class="content">
        <div class="m-b-lg">
          ...
          <p>A chart displaying all the recorded fireballs and bolides events from NASA's open API.</p>
          <p>A log scale is applied to the radius calculation due to the massive
            <a href="https://en.wikipedia.org/wiki/Chelyabinsk_event" target="new">Chelyabinsk Event</a>
            being such an extreme outlier in terms of energy radiated. (That's the big fella close to the
          ...
          <p>
            ...
            <a @click="setChartToRadiatedLinear">Set the chart to a linear scale showing energy radiated</a>
          </p>
        </div>
      </div>
      <h4 class="has-text-weight-bold has-text-white m-b-sm">Scrub by date range</h4>
      <fireball-slider :is-visible="showDrawer" :date-range="fireballYearRange" class="m-b-xl" />
      ...
      <div v-if="fireballs" class="tabs is-toggle is-fullwidth">
        <ul>
          <li v-for="scale in scales" :key="scale.id" :class="{ 'is-active': scaleToPlot === scale.id }"
            @click="setScaleToPlot(scale.id)" :data-test="`scaleToggle-${scale.id}`">
            <a class="has-text-white">
